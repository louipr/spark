...
// Bootstrap application, handle CLI arguments, and coordinate all major components
...
class SparkApplication {
  ...
  private initializeComponents(): void {
    try {
      ...
      const storagePath = expandHomePath(process.env.STORAGE_PATH || '~/.spark-clone');
      
      this.logger = new Logger({
        level: process.env.LOG_LEVEL as any || 'info',
        enableFile: true,
        filePath: path.join(storagePath, 'logs')
      });
